...
Scene* GameScene2::createScene(int s)
{
...
bool GameScene2::init()
{
	...
	pauseButton->addTouchEventListener(CC_CALLBACK_2(GameScene2::menupauseCallback, this));
	// 暂停界面的返回按钮
	auto pausetBackButton = (Button*)gameUI->getChildByName("pause_back_to_game");
	pausetBackButton->addTouchEventListener(CC_CALLBACK_2(GameScene2::menubackCallback, this));
	// 退出按钮
	auto quitButton = (Button*)gameUI->getChildByName("quit_button");
	quitButton->addTouchEventListener(CC_CALLBACK_2(GameScene2::menuquitCallback, this));
	// 退出界面的返回按钮
	auto quitBackButton = (Button*)gameUI->getChildByName("quit_back_to_game");
	quitBackButton->addTouchEventListener(CC_CALLBACK_2(GameScene2::menubackCallback, this));
	// 退出界面的确定按钮
	auto quitSureButton = (Button*)gameUI->getChildByName("sure_to_quit");
	quitSureButton->addTouchEventListener(CC_CALLBACK_2(GameScene2::menuquitgameCallback, this));
	// 力度条
	...
	listener->onTouchEnded = CC_CALLBACK_2(GameScene2::onTouchEnd, this);
	listener->onTouchBegan = CC_CALLBACK_2(GameScene2::onTouchBegin, this);
	listener->onTouchMoved = CC_CALLBACK_2(GameScene2::onTouchMove, this);
	...
	if (playerArrow.islegal == false && !ispause)
	{
		...
		for (auto target : targetVector)
		{
			if (playerArrow.arrow->getBoundingBox().intersectsRect(target->getBoundingBox()))
			{
				...
				if (playerArrow.bonus == 0 || playerArrow.bonus % 3 != 0)
				{
					...
					if (playerArrow.bonus % 3 == 0) {
						...
						bonusLabel->runAction(Sequence::create(
							...
							CallFunc::create(CC_CALLBACK_0(GameScene2::flyout, this, bonusLabel)),
							NULL
						));
					}
				}
				...
				// 箭靶命中后消失
				auto actionDown = CallFunc::create(CC_CALLBACK_0(GameScene2::removeTarget, this, target));
				target->runAction(actionDown);
				tmp->removeFromParent();

				// 箭靶增加
				auto actionAdd = CallFunc::create(CC_CALLBACK_0(GameScene2::addTarget, this));
...
void GameScene2::onTouchEnd(Touch* touch, Event* unused_event)
{
...
bool GameScene2::onTouchBegin(Touch* touch, Event* unused_event)
{
	if (playerArrow.islegal == true && !ispause)
	{
	...
	else if (playerArrow.islegal == false && !ispause)
	{
		...
		label->runAction(Sequence::create(
			...
			CallFunc::create(CC_CALLBACK_0(GameScene2::flyout, this, label)),
			NULL
		));
		return false;
	}
...
void GameScene2::onTouchMove(Touch* touch, Event* unused_event)
{
	if (playerArrow.islegal == true && !ispause)
	{
...
void GameScene2::removeTarget(Sprite* target)
{
	this->removeChild(target);
	targetVector.eraseObject(target);
}

void GameScene2::flyout(Label* label) {
	label->setVisible(false);
	label->removeFromParent();
}

void GameScene2::randomWind()
{
	wind = random(-1, 1);
...
void GameScene2::showScoreTips(int score, Vec2 position, Color4B color)
{
	playerScore += score;
...
void GameScene2::menuquitCallback(cocos2d::Ref* pSender, cocos2d::ui::Widget::TouchEventType type)
{
	switch (type)
	{
...
void GameScene2::menupauseCallback(cocos2d::Ref* pSender, cocos2d::ui::Widget::TouchEventType type)
{
	switch (type)
	{
	case cocos2d::ui::Widget::TouchEventType::BEGAN:
...
void GameScene2::menubackCallback(cocos2d::Ref* pSender, cocos2d::ui::Widget::TouchEventType type)
{
	switch (type)
	{
	case cocos2d::ui::Widget::TouchEventType::BEGAN:
...
void GameScene2::menuquitgameCallback(cocos2d::Ref* pSender, cocos2d::ui::Widget::TouchEventType type)
{
	switch (type)
	{
	case cocos2d::ui::Widget::TouchEventType::BEGAN:
